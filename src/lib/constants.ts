...
export async function generatePreviewUrl(key: string, duration: number = 4): Promise<string> {
...
function audioBufferToPreviewBlob(buffer: AudioBuffer): Blob {
  ...
  w(0, 'RIFF'); v.setUint32(4, 36 + ds, true);
  w(8, 'WAVE'); w(12, 'fmt ');
  ...
  for (let i = 0; i < ns; i++) {
    for (let ch = 0; ch < nc; ch++) {
      v.setInt16(44 + (i * nc + ch) * bps,
        Math.max(-32768, Math.min(32767, Math.round(buffer.getChannelData(ch)[i] * 32767))), true);
    }
  }
