...
function noteNameFromFreq(freq: number): {
  name: string;
  octave: number;
  cents: number;
} {
...
export function Tuner({ visible, onClose, testID }: TunerProps) {
  ...
  useEffect(() => {
    ...
    animRef.current = setInterval(() => {
      ...
      const freq =
        40 +
        Math.sin(step * 0.3) * 200 +
        Math.sin(step * 0.7) * 50 +
        100 * Math.sin(step * 0.1);
      setSimFreq(Math.max(20, freq + 200));

      const note = noteNameFromFreq(freq + 200);
      const idx = tuningRef.current.findIndex(
        (t) => t.name === note.name && Math.abs(t.octave - note.octave) <= 1,
      );
      ...
      >
        <Pressable className="w-full max-w-sm bg-dark-surface rounded-3xl border border-dark-border p-6">
          ...
          <View className="flex-row gap-2 mb-6">
            ...
            >
              <Text
                className={`text-sm font-semibold ${instrument === "guitar" ? "text-brand-primary" : "text-white"}`}
              >
                🎸 Guitarra
              </Text>
            </Pressable>
            ...
            >
              <Text
                className={`text-sm font-semibold ${instrument === "bass" ? "text-brand-accent" : "text-white"}`}
              >
                🎸 Baixo
              </Text>
            </Pressable>
          ...
          <View className="items-center mb-6">
            <Text
              className={`text-6xl font-bold ${isInTune ? "text-green-400" : note.cents > 0 ? "text-yellow-400" : "text-brand-primary"}`}
            >
              {note.name}
              {note.octave}
            </Text>
            <Text
              className={`text-sm font-mono mt-1 ${isInTune ? "text-green-400" : "text-gray-400"}`}
            >
              {simFreq > 0 ? `${simFreq.toFixed(1)} Hz` : "—"}
            </Text>
            <Text
              className={`text-xs mt-0.5 font-semibold ${isInTune ? "text-green-400" : note.cents > 0 ? "text-yellow-500" : "text-brand-primary"}`}
            >
              {simFreq > 0
                ? isInTune
                  ? "✓ Afinado"
          ...
          <Text className="text-gray-500 text-[10px] font-semibold uppercase tracking-wider mb-2">
            Referência
          </Text>
          <View className="flex-row flex-wrap gap-1.5">
            {tuning.map((t, i) => (
              ...
              >
                <Text
                  className={`text-xs font-bold ${activeString === i ? "text-brand-accent" : "text-white"}`}
                >
                  {t.name}
                  {t.octave}
                </Text>
                <Text className="text-gray-600 text-[8px]">
