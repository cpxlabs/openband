  ...
  return (
    <View testID={testID}>
      ...
      >
        ...
        <View className={`flex-1 h-1 rounded-full bg-dark-border overflow-hidden ${activeMixId ? 'opacity-100' : 'opacity-50'}`}>
          {activeMixId && (
            <View className="h-full bg-brand-accent rounded-full" style={{ width: '60%' }} />
          )}
        </View>
        ...
        <View className="absolute top-full mt-2 right-0 w-80 bg-dark-elevated border border-dark-border rounded-2xl p-4 z-50 shadow-xl">
          <View className="flex-row items-center justify-between mb-3">
            ...
            {compareIds.length === 2 && (
              ...
              >
                <Text className="text-brand-accent text-xs font-bold">Comparar A/B</Text>
              </Pressable>
            )}
          </View>
          ...
          {showSave ? (
            ...
            >
              <Text className="text-brand-accent text-xs font-bold">+ Salvar Mix Atual</Text>
            </Pressable>
          )}
          ...
          <ScrollView style={{ maxHeight: 200 }}>
            {snapshots.length === 0 ? (
              <View className="py-6 items-center">
                ...
                <Text className="text-gray-700 text-[10px] mt-1">Salve snapshots para comparar</Text>
              </View>
            ) : (
              snapshots.map(snap => {
                  ...
                  >
                    <Pressable
                      ...
                      className={`w-5 h-5 rounded border items-center justify-center ${
                        isSelected ? 'bg-emerald-500 border-emerald-400' : 'border-dark-border'
                      }`}
                    >
                    ...
                    <View className="flex-1">
                      <Text className={`text-xs font-semibold ${isActive ? 'text-brand-accent' : 'text-gray-200'}`}>
                        {snap.name}
                      </Text>
                    ...
                    >
                      <Text className={`text-[10px] font-bold ${isActive ? 'text-white' : 'text-gray-400'}`}>
