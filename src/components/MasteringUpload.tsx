...
import { formatSampleRate, formatBitDepth, formatFileSize } from '../lib/masteringSuite';
  ...
  return (
    <View testID={testID} className="bg-dark-surface rounded-xl border border-dark-border p-4">
      ...
      <View className="flex-row gap-2 mb-3">
        ...
        >
          <Text className={`text-xs font-bold ${mode === 'single' ? 'text-green-400' : 'text-gray-400'}`}>Single File</Text>
          <Text className={`text-[8px] mt-0.5 ${mode === 'single' ? 'text-green-400/70' : 'text-gray-600'}`}>Mix estéreo .wav</Text>
        </Pressable>
        <Pressable
        ...
        >
          <Text className={`text-xs font-bold ${mode === 'stems' ? 'text-green-400' : 'text-gray-400'}`}>Stems</Text>
          <Text className={`text-[8px] mt-0.5 ${mode === 'stems' ? 'text-green-400/70' : 'text-gray-600'}`}>Multi-track .wav</Text>
        </Pressable>
      </View>
      ...
      {input ? (
        <View className="bg-dark-elevated rounded-lg border border-dark-border/50 p-3">
          <View className="flex-row items-center justify-between">
            <View className="flex-1">
              <Text className="text-white text-xs font-semibold">{input.filename}</Text>
              <View className="flex-row gap-3 mt-1">
                <Text className="text-gray-500 text-[9px]">{formatSampleRate(input.sampleRate)}</Text>
                <Text className="text-gray-500 text-[9px]">{formatBitDepth(input.bitDepth)}</Text>
                <Text className="text-gray-500 text-[9px]">{formatFileSize(input.size)}</Text>
              </View>
              {mode === 'stems' && input.stems ? (
                <View className="flex-row gap-2 mt-1.5 flex-wrap">
                  {input.stems.map(s => (
                    <View key={s.name} className="px-2 py-0.5 rounded bg-dark-muted/50">
            ...
            <Pressable onPress={onClear} className="px-2.5 py-1 rounded bg-red-500/20 border border-red-500/30">
              <Text className="text-red-400 text-[10px] font-medium">Trocar</Text>
            </Pressable>
          </View>
        </View>
      ) : (
        ...
        >
          ...
          <Text className="text-gray-600 text-[10px] mt-1">
            {mode === 'single' ? '16-bit/44.1kHz ~ 32-bit/96kHz' : 'Drums, Bass, Vocals, Melodies...'}
          </Text>
        </Pressable>
      )}
    </View>
