...
export function PluginRack({
  ...
  const addPlugin = useCallback(
    (preset: Partial<Plugin>) => {
      if (plugins.length >= maxSlots) return;
      const type = PLUGIN_SPECS[preset.type as keyof typeof PLUGIN_SPECS]
        ? (preset.type as Plugin["type"])
        : "utility";
      const plugin: Plugin = {
  ...
  return (
    <View
      testID={testID}
      className="bg-dark-surface/80 rounded-xl border border-dark-border"
    >
      <Pressable
            ...
            <View className="flex-row -space-x-1">
              {plugins
                ...
                .map((p) => (
                  <View
                    key={p.id}
                    className="w-2 h-2 rounded-full border border-dark-bg"
                    style={{ backgroundColor: p.color }}
                  />
                ))}
            </View>
          )}
          <Text className="text-brand-accent text-xs font-bold ml-1">
            {showAdd ? "−" : "+"}
          </Text>
        </View>
      </Pressable>
      ...
      {plugins.map((plugin, index) => (
        ...
        >
          ...
          <View className="flex-1">
            <Text
              className={`text-xs font-semibold ${plugin.enabled ? "text-gray-200" : "text-gray-600"}`}
            >
              {plugin.name}
            </Text>
            <Text
              className={`text-[9px] ${plugin.enabled ? "text-gray-500" : "text-gray-700"}`}
            >
              ...
              return (
                ...
                >
                  ...
                  <Text className="text-gray-500 text-[9px] mt-0.5">
                    {PLUGIN_ICONS[presetType] || "≡"}{" "}
                    {formatPluginType(presetType).toLowerCase()}
                  </Text>
                </Pressable>
              );
            })}
      ...
      {plugins.length === 0 && !showAdd && (
        <View className="py-3 items-center">
          ...
          <Pressable onPress={() => setShowAdd(true)} className="mt-1">
