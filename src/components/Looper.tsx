...
export function Looper({ visible, onClose, bpm, onCommitLoop }: LooperProps) {
  ...
  const toggleRecord = useCallback((slotId: number) => {
    setSlots(prev => prev.map(s => {
      if (s.id !== slotId) return s;
      if (s.recording) {
        onCommitLoop(slotId, s.bars);
        return { ...s, recording: false, hasContent: true, layers: s.layers + 1 };
      }
  ...
  const clearSlot = useCallback((slotId: number) => {
    setSlots(prev => prev.map(s => s.id === slotId ? { ...s, hasContent: false, layers: 0, recording: false } : s));
      ...
      <View className="flex-1 bg-black/80 justify-center items-center px-4">
        <View className="w-full max-w-sm bg-dark-surface rounded-3xl border border-dark-border p-5">
          <View className="flex-row items-center justify-between mb-4">
            ...
            <Text className="text-gray-400 text-xs">{bpm} BPM · {beatDuration.toFixed(2)}s/beat</Text>
          </View>
          ...
          {slots.map(slot => (
            <View
              ...
              style={{
                borderColor: slot.recording ? '#ff375f' : activeSlot === slot.id ? SLOT_COLORS[slot.id] : '#333',
                backgroundColor: slot.recording ? 'rgba(255,55,95,0.1)' : 'rgba(255,255,255,0.03)',
              }}
            >
              <View className="flex-row items-center justify-between mb-2">
                <View className="flex-row items-center gap-2">
                  <View className="w-3 h-3 rounded-full" style={{ backgroundColor: SLOT_COLORS[slot.id] }} />
                  <Text className="text-white text-sm font-semibold">Loop {slot.id + 1}</Text>
                  {slot.hasContent && (
                    <View className="px-2 py-0.5 rounded-full bg-green-500/20 border border-green-500/30">
                      <Text className="text-green-400 text-[9px]">{slot.layers} layer{slot.layers > 1 ? 's' : ''}</Text>
                    </View>
                  )}
                </View>
                {slot.recording && (
                  <View className="flex-row items-center gap-1">
                    ...
                    <Text className="text-red-400 text-[10px] font-semibold">REC</Text>
                  </View>
                )}
              </View>
              ...
              <View className="flex-row items-center gap-2 mb-2">
                ...
                <View className="flex-row gap-1">
                  {BAR_OPTIONS.map(b => (
                    ...
                    >
                      <Text className={`text-[10px] ${slot.bars === b ? 'text-brand-accent' : 'text-gray-400'}`}>{b}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
              ...
              <View className="flex-row gap-2">
                <Pressable
                  ...
                  style={{ backgroundColor: slot.recording ? '#ff375f' : '#1c1c1e' }}
                >
                  <Text className="text-white text-xs font-bold">{slot.recording ? 'Stop' : slot.hasContent ? 'Overdub' : 'Record'}</Text>
                </Pressable>
                {slot.hasContent && (
                  <Pressable
          ...
          <Pressable onPress={onClose} className="mt-2 py-3 rounded-xl border border-dark-border items-center active:opacity-70">
