    ...
    >
      ...
      >
        <View className="flex-row items-center gap-2">
          ...
          <View className="bg-brand-accent/20 px-1.5 py-0.5 rounded">
            <Text className="text-brand-accent text-[8px] font-bold">
              {MASTERING_CHAIN_PRESETS[activePreset]?.name || "Off"}
            </Text>
          </View>
        </View>
      ...
      {expanded && (
        <View className="p-3 gap-3">
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View className="flex-row gap-1.5 pb-1">
              {MASTERING_CHAIN_PRESETS.map((preset, i) => (
                ...
                >
                  <Text
                    className={`text-[9px] font-semibold ${activePreset === i ? "text-brand-accent" : "text-gray-300"}`}
                    style={{ flexShrink: 0 }}
                  >
                    {preset.name}
                  </Text>
          ...
          <Text className="text-gray-500 text-[9px] font-semibold uppercase tracking-wider">
            EQ Rápido
          </Text>
              ...
              return (
                <View key={band.id} className="flex-row items-center gap-2">
                  ...
                  <View className="flex-row items-center gap-0.5">
                    <Pressable
                      onPress={() =>
                        onEqChange(band.id, Math.max(-12, val - 1))
                      }
                    ...
                    <Text className="text-white text-[9px] font-mono w-7 text-center">
                      {val > 0 ? "+" : ""}
                      {val.toFixed(0)}
                    </Text>
                    <Pressable
                      onPress={() => onEqChange(band.id, Math.min(12, val + 1))}
                      className="w-5 h-5 rounded bg-dark-muted items-center justify-center active:opacity-70"
                    >
                      <Text className="text-gray-400 text-[9px]">+</Text>
                    </Pressable>
