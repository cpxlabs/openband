...
const PRESET_PATTERNS: { name: string; code: string }[] = [
  { name: '808 Classic', code: 'KICK SNARE HH KICK SNARE HH KICK SNARE HH KICK SNARE HH' },
  { name: 'Trap Roll', code: 'KICK CLAP HH HH KICK CLAP HH HH KICK CLAP HH OH KICK CLAP HH HH' },
  { name: 'Hip Hop', code: 'KICK SNARE HH KICK KICK SNARE HH KICK SNARE HH HH KICK SNARE OH' },
  { name: 'Four on Floor', code: 'KICK HH SNARE HH KICK HH SNARE HH KICK HH SNARE HH KICK HH SNARE HH' },
  { name: 'Half Time', code: 'KICK SNARE KICK KICK SNARE KICK SNARE KICK' },
  { name: 'Bassline Basic', code: 'BASS REST BASS REST BASS REST BASS REST BASS REST BASS REST BASS REST BASS REST' },
];

function parsePattern(code: string): TokenKey[] {
  return code.trim().toUpperCase().split(/\s+/).filter(t => t === 'REST' || TOKEN_MAP[t as TokenKey]) as TokenKey[];
}
...
export function CodeSampler({ visible, onClose, onRender, bpm }: CodeSamplerProps) {
  ...
  const stepSeconds = unit === '1/4' ? 60 / effectiveBpm : unit === '1/8' ? 30 / effectiveBpm : 15 / effectiveBpm;
      ...
      <Pressable className="flex-1 bg-black/60 justify-center items-center px-3" onPress={onClose}>
        <Pressable className="w-full max-w-lg max-h-[80%] bg-dark-surface rounded-3xl border border-dark-border overflow-hidden" onPress={() => {}}>
          <View className="p-5 border-b border-dark-border">
            ...
            <Text className="text-gray-500 text-xs">Escreva padrões com código para gerar faixas</Text>
          </View>
          ...
          <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
            ...
            <View className="flex-row flex-wrap gap-1.5 mb-4">
              {(Object.keys(TOKEN_MAP) as TokenKey[]).map(key => (
                <View key={key} className={`${TOKEN_MAP[key].color} px-2 py-0.5 rounded-md flex-row items-center gap-1`}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-4">
              <View className="flex-row gap-2">
                {PRESET_PATTERNS.map(p => (
                  <Pressable key={p.name} onPress={() => handlePreset(p.code)}
                    className="px-3 py-2 rounded-xl bg-dark-elevated border border-dark-border active:opacity-70">
                    <Text className="text-gray-300 text-xs font-semibold">{p.name}</Text>
                  </Pressable>
                ))}
              </View>
            </ScrollView>
            ...
            <View className="flex-row items-center gap-3 mb-4">
              <View className="flex-row items-center gap-1.5">
                <Text className="text-gray-400 text-[10px] font-semibold">Step:</Text>
                {(['1/4', '1/8', '1/16'] as StepUnit[]).map(u => (
                  <Pressable key={u} onPress={() => setUnit(u)}
                    ...
                    <Text className={`text-[10px] font-semibold ${unit === u ? 'text-brand-accent' : 'text-white'}`}>{u}</Text>
                  </Pressable>
                ))}
              </View>
              {!useBpm && (
                <View className="flex-row items-center gap-1">
                  <Text className="text-gray-400 text-[10px] font-semibold">BPM:</Text>
                  <TextInput
              ...
              <Pressable onPress={() => setUseBpm(!useBpm)}
                className={`px-2 py-1 rounded-lg border ${useBpm ? 'bg-brand-accent/20 border-brand-accent' : 'bg-dark-elevated border-dark-border'}`}>
                <Text className={`text-[10px] font-semibold ${useBpm ? 'text-brand-accent' : 'text-gray-400'}`}>Seg BPM</Text>
              </Pressable>
            </View>

            <Text className="label mb-2">Preview — {tokens.length} steps ({totalDuration.toFixed(1)}s @ {effectiveBpm} BPM)</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={true} className="mb-4">
              <View className="flex-row gap-0.5 py-1">
                {tokens.map((t, i) => (
                  <View key={i} className={`w-10 h-10 rounded-lg ${TOKEN_MAP[t]?.color || 'bg-dark-muted'} items-center justify-center ${i % 4 === 3 ? 'mr-0.5' : ''}`}>
                    <Text className="text-white text-[8px] font-bold text-center leading-tight">{t}</Text>
                  </View>
                ))}
              </View>
            </ScrollView>
