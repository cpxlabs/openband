...
function PedalSlot({
  ...
  pedal: GuitarPedal | null;
  index: number;
  onToggle: () => void;
  onReplace: (p: GuitarPedal) => void;
  onRemove: () => void;
}) {
    ...
    <View className="w-20 items-center gap-1">
      ...
      >
        {pedal ? (
          <View className="items-center">
            <View
              className={`w-3 h-3 rounded-full mb-0.5 ${pedal.enabled ? "bg-green-500" : "bg-gray-600"}`}
            />
            <Text className="text-[8px] text-gray-400 text-center leading-tight px-0.5">
              {pedal.name}
            </Text>
      ...
      {pedal && (
        <View className="flex-row gap-1">
          ...
          <Pressable
            onPress={onRemove}
            className="px-1.5 py-0.5 rounded bg-red-500/20 active:opacity-70"
          >
      ...
      {showPicker && (
        <View
          className="absolute top-full left-0 z-50 w-48 bg-dark-elevated border border-dark-border rounded-xl p-2 shadow-2xl"
          style={{ maxHeight: 200 }}
        >
          <ScrollView showsVerticalScrollIndicator={false}>
            {PEDAL_PRESETS.map((preset, i) => (
              ...
              >
                <Text className="text-[10px] text-gray-400 flex-1">
                  {preset.brand} {preset.name}
                </Text>
                <Text className="text-[8px] text-gray-600 uppercase">
                  {preset.type}
                </Text>
              </Pressable>
            ))}
...
function AmpSelector({
  ...
  const filtered = selectedBrand
    ? AMP_PRESETS.filter((a) => a.brand === selectedBrand)
    : AMP_PRESETS;
  return (
    <View className="relative">
                ...
                <View key={k} className="flex-1 items-center">
                  <View className="h-6 w-full bg-dark-bg rounded-full overflow-hidden">
                    <View
                      ...
                      style={{ height: `${(amp.params[k] / 10) * 100}%` }}
                    />
                  </View>
                  <Text className="text-[6px] text-gray-600 uppercase mt-0.5">
                    {k[0]}
                  </Text>
                </View>
              ))}
      ...
      {amp && (
        <Pressable
          onPress={onRemove}
          className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-red-500/30 items-center justify-center"
        >
      ...
      {showPicker && (
        <View
          className="absolute top-full left-0 z-50 w-64 bg-dark-elevated border border-dark-border rounded-xl p-2 shadow-2xl"
          style={{ maxHeight: 260 }}
        >
          <ScrollView
            horizontal
          ...
          >
            <View className="flex-row gap-1">
              ...
              >
                <Text
                  className={`text-[9px] ${!selectedBrand ? "text-orange-400" : "text-gray-400"}`}
                >
                  Todos
                </Text>
              </Pressable>
              {brands.map((b) => (
                ...
                >
                  <Text
                    className={`text-[9px] ${selectedBrand === b ? "text-orange-400" : "text-gray-400"}`}
                  >
                    {b}
                  </Text>
                </Pressable>
...
function CabSelector({
  ...
  const [selectedBrand, setSelectedBrand] = useState<string | null>(null);
  const filtered = selectedBrand
    ? CAB_PRESETS.filter((c) => c.brand === selectedBrand)
    : CAB_PRESETS;
  return (
    <View className="relative">
      <Pressable
      ...
      >
        {cab ? (
              ...
              <View className="flex-1">
                ...
                <View className="h-1.5 bg-dark-bg rounded-full overflow-hidden">
                  <View
                    ...
                    style={{ width: `${cab.params.micPosition}%` }}
                  />
                </View>
              </View>
              <View className="flex-1">
          ...
          <View className="items-center py-2">
            <Text className="text-[9px] text-gray-500 font-bold uppercase">
              Cab
            </Text>
            <Text className="text-[16px] text-gray-600">+</Text>
          </View>
        ...
        >
          ...
          >
            <View className="flex-row gap-1">
              ...
              >
                <Text
                  className={`text-[9px] ${!selectedBrand ? "text-amber-400" : "text-gray-400"}`}
                >
                  Todos
                </Text>
              </Pressable>
              {brands.map((b) => (
                ...
                >
                  <Text
                    className={`text-[9px] ${selectedBrand === b ? "text-amber-400" : "text-gray-400"}`}
                  >
                    {b}
                  </Text>
                </Pressable>
...
export function PedalRack({
  ...
  const handleTogglePedal = (index: number) => {
    ...
    if (pedals[index])
      pedals[index] = { ...pedals[index], enabled: !pedals[index].enabled };
      ...
      >
        <View className="flex-row gap-2 py-1">
          ...
          <View className="w-36">
            <AmpSelector
              amp={chain.amp}
              onSelect={(a) => onChange({ ...chain, amp: a })}
              onRemove={() => onChange({ ...chain, amp: null })}
            />
          </View>
          <View className="w-36">
            <CabSelector
              cab={chain.cab}
              onSelect={(c) => onChange({ ...chain, cab: c })}
              onRemove={() => onChange({ ...chain, cab: null })}
            />
          </View>
        </View>
      </ScrollView>
