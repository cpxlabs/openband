      ...
      {expanded && (
        <View className="absolute top-full mt-2 right-0 w-72 bg-dark-elevated border border-dark-border rounded-2xl p-4 z-50 shadow-xl">
          <View className="flex-row items-center justify-between mb-3">
            ...
            >
              <View className={`w-5 h-5 rounded-full bg-white shadow-sm ${settings.enabled ? 'ml-6' : 'ml-0'}`} />
            </Pressable>
          </View>
          ...
          <View className="flex-row items-center gap-2 mb-3">
            <Pressable
              onPress={() => onChange({ ...settings, bpm: Math.max(40, settings.bpm - 1) })}
            ...
            />
            <Pressable
              onPress={() => onChange({ ...settings, bpm: Math.min(280, settings.bpm + 1) })}
              className="w-8 h-8 rounded-lg bg-dark-surface items-center justify-center active:opacity-70"
            >
              <Text className="text-gray-300 text-lg">+</Text>
            </Pressable>
          ...
          <View className="flex-row flex-wrap gap-1.5 mb-3">
            {TIME_SIGS.map(sig => {
              const active = settings.timeSig[0] === sig[0] && settings.timeSig[1] === sig[1];
              return (
                <Pressable
                  key={`${sig[0]}/${sig[1]}`}
                ...
                >
                  <Text className={`font-mono text-xs ${active ? 'text-brand-primary' : 'text-gray-400'}`}>
                    {sig[0]}/{sig[1]}
                  </Text>
                </Pressable>
              );
          ...
          <View className="flex-row gap-1.5 mb-3">
            {[1, 2, 4].map(n => (
              ...
              >
                <Text className={`font-mono text-xs ${settings.accentInterval === n ? 'text-emerald-400' : 'text-gray-400'}`}>
                  {n} tempo{n > 1 ? 's' : ''}
                </Text>
              </Pressable>
            ))}
          ...
          <View className="flex-row gap-1.5 mb-3">
            {[0, 1, 2, 4].map(n => (
              <Pressable
                key={n}
                onPress={() => onChange({ ...settings, countIn: n > 0, countInBars: Math.max(n, 1) })}
              ...
              >
                <Text className={`font-mono text-xs ${settings.countIn && settings.countInBars === Math.max(n, 1) ? 'text-brand-accent' : 'text-gray-400'}`}>
          ...
          <View className="flex-row items-center gap-2">
            ...
            <View className="flex-1 h-1.5 bg-dark-border rounded-full overflow-hidden">
              <View className="h-full bg-emerald-500 rounded-full" style={{ width: `${settings.volume}%` }} />
            </View>
            <Text className="text-gray-400 font-mono text-xs w-8 text-right">{settings.volume}%</Text>
            <Pressable
              onPress={() => onChange({ ...settings, volume: Math.min(100, settings.volume + 10) })}
            ...
            <View className="flex-row gap-1 mt-3 justify-center">
              {Array.from({ length: settings.timeSig[0] }, (_, i) => (
                <View
                  ...
                  style={{ opacity: 0.6 + Math.sin(Date.now() * 0.008 + i * 2) * 0.4 }}
                />
              ))}
            </View>
          )}
