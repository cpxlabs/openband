...
export const Empty: Story = {
  args: {
    groups: [],
    tracks: mockTracks,
    onCreateGroup: (name: string, ids: string[]) => alert(`Create group ${name} with ${ids.length} tracks`),
    ...
    onAssignTrack: (trackId: string, groupId: string | null) => console.log('Assign', trackId, groupId),
    trackAssignments: {},
  },
};
...
export const WithGroups: Story = {
  args: {
    groups: [
      { id: 'g1', name: 'Bateria', color: '#ff6482', volume: 80, muted: false, trackIds: ['t4'] },
      { id: 'g2', name: 'Guitarras', color: '#5ac8fa', volume: 65, muted: false, trackIds: ['t2'] },
    ],
    tracks: mockTracks,
    onCreateGroup: (name: string, ids: string[]) => alert(`Create group ${name} with ${ids.length} tracks`),
    onRemoveGroup: (id: string) => alert(`Remove ${id}`),
