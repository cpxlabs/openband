...
const ALLOWED_ORIGINS = [
  ...
  'exp://localhost:19000', 'exp://localhost:19001', 'http://127.0.0.1:8081',
  /\.vercel\.app$/,
];
app.use(cors({
...
setInterval(() => {
  ...
  if (Object.keys(rateLimitStore).length > RATE_LIMIT_MAX_ENTRIES) {
    ...
    for (const [ip] of sorted.slice(0, sorted.length - RATE_LIMIT_MAX_ENTRIES)) delete rateLimitStore[ip];
  }
}, 60 * 1000);
